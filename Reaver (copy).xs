...
size_t
build_association_management_frame(struct association_request_management_frame *f)

size_t
build_authentication_management_frame(struct authentication_management_frame *f)

size_t
 build_ssid_tagged_parameter(unsigned char buf[IW_ESSID_MAX_SIZE+2], char *essid)
size_t
 build_supported_rates_tagged_parameter(unsigned char *buf, size_t buflen)
size_t
 build_wps_tagged_parameter(unsigned char buf[2+WPS_TAG_SIZE])
size_t
 build_htcaps_parameter(unsigned char *buf, size_t buflen)

void
 *build_wps_probe_request(unsigned char *bssid, char *essid, size_t *len)
void
 *build_snap_packet(size_t *len)
void
 *build_dot1X_header(uint8_t type, uint16_t payload_len, size_t *len)
void
